...
pal_ev_loop_t *pal_ev_create_loop(int max_timeout)
{
    ...
    if ((loop = (pal_ev_loop_t *)malloc(sizeof(pal_ev_loop_t))) == NULL) {
        return NULL;
    }
    ...
    for (i = 0; i < pal_ev.max_fd; ++i) {
        ...
        if (fd->loop_id == loop->loop_id) {
            if ((fd->events & PAL_EV_READ) != 0) {
                PAL_EV_FD_SET(i, &readfds);
                if (maxfd < i) {
                    maxfd = i;
                }
            }
            if ((fd->events & PAL_EV_WRITE) != 0) {
                PAL_EV_FD_SET(i, &writefds);
                if (maxfd < i) {
                    maxfd = i;
                }
            }
    ...
    if (r == -1) {
        ...
        for (i = 0; i < pal_ev.max_fd; ++i) {
            ...
            if (target->loop_id == loop->loop_id) {
                int revents = (PAL_EV_FD_ISSET(i, &readfds) ? PAL_EV_READ : 0) | (PAL_EV_FD_ISSET(i, &writefds) ? PAL_EV_WRITE : 0);
                if (revents != 0) {
