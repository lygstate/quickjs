                                          ...
                                          JSValue var, JSValue var_val)
{
    ...
    if (JS_IsString(var_val))
        ...
        // xor the the two dwords to get a better hash.
        uint64_t intp = (uintptr_t)p;
        uint32_t pl = (uint32_t)((intp >> 32) ^ (intp));
...
static int js_line_equal(int current_line, int expected_line)
{
    return (current_line == -1) || (current_line == expected_line);
}
...
void js_debugger_check(JSContext *ctx, const uint8_t *cur_pc)
{
    ...
    if (info->stepping) {
        ...
        if (info->step_depth == depth && location.filename == info->step_over.filename && js_line_equal(location.line, info->step_over.line) && location.column == info->step_over.column)
            goto done;
    }
    ...
    if (at_breakpoint) {
        ...
        if (info->stepping == JS_DEBUGGER_STEP_CONTINUE) {
            ...
            if (info->step_depth == depth) {
                ...
                if (location.filename == info->step_over.filename && js_line_equal(location.line, info->step_over.line) && location.column == info->step_over.column)
                    goto done;
            }
            info->stepping = 0;
            info->is_paused = 1;
            ...
            if ((location.filename == info->step_over.filename && js_line_equal(location.line, info->step_over.line) && location.column == info->step_over.column) || js_debugger_stack_depth(ctx) > info->step_depth)
                goto done;
            info->stepping = 0;
            info->is_paused = 1;
...
void js_debugger_attach(
    JSContext *ctx,
    int (*transport_read)(void *udata, char *buffer, int length),
    int (*transport_write)(void *udata, const char *buffer, int length),
    int (*transport_peek)(void *udata),
