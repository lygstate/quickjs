...
bool pal_isnan(double d);
bool pal_isfinite(double d);
bool pal_isinf(double d);
 ...
 * @param unix_ms Unix time in ms
 * @param is_utc If the unix time is UTC time
 * @param
 * @return timezone offset in second
 *  For example
 *   GTM+8 will return 8 hours= 8 * 60 * 60 seconds
 *   UTC  will return 0
 *   GTM-8 will return -8 hours= -8 * 60 * 60 seconds
 */
...
/**
 * @brief get timezone offset
 * @param utc_time_ms UTC epoch time in ms
 * @param timezone_offset The timezone offset
 * @return if setting succeed
 */
int pal_settimezoneoffset(int64_t utc_time_ms, int timezone_offset);
