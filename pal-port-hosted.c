...
void pal_initialize(int argc, void **argv, void **envp, bool is_wchar)
{
  ...
  if (is_wchar) {
    ...
    global_pal.argv = (wchar_t **)argv;
    global_pal.envp = (wchar_t **)envp;
  } else {
  ...
  info->executable_path = pal_process_executable_path();
  info->executable_dir = pal_dirname(info->executable_path);
}
...
pal_session_t *pal_opensession()
{
  ...
  pal->header.cwd_length = (uint32_t)strlen(pal->header.cwd);
